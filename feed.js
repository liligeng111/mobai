var debug_feed = false; // debug_feed mode

function check_mobai_in_feed()
{	
	var $ = jQuery;
	var feeds = $('.feed-content');
	if (debug_feed) console.log('inside feed function');
	feeds.each(function(i, item)
	{
		...
		if (debug_feed) console.log('try to get feed ' + i + ' from: ' + url);
		
		$.get(url, function(data)  // the page when you click the link
		{
			if (debug_feed) console.log('reply page is get');
			...
			$.post("http://www.renren.com/feedcommentretrieve.do",  // the actual comments
				...
				_rtk:rtk
				}, function(data)
					{
						...
						if (debug_feed) console.log('search from ' + obj.replyList.length + ' elements:');
						if (debug_feed) console.log(obj);
						for (var j = obj.replyList.length - 1; j > -1; j--)
						{
							...
							if(obj.replyList[j].ubid == replyer)
							{
								...
								{
									if (debug_feed) console.log('no (mb) found');
									return;
								}
								
								if (debug_feed) console.log('bingo');
								if (debug_feed) console.log($(item).find('h4'));
		...
		if (debug_feed) console.log('end of feed ' + i);
	});
	if (debug_feed) console.log('end of feed function');
}
...
function check_feed_is_ready()
{
	...
	if (feeds.length == 0)
	{
		if (debug_feed) console.log('feed is not ready');
		if(document_ready)
		{			
			if (debug_feed) console.log('document is ready, no feed!');
			return;
		}
	...
	if (debug_feed) console.log('feed is ready, begin to load feeds');
	check_mobai_in_feed();
}
